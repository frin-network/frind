...
import (
	...
	"github.com/frin-network/frind/domain/consensus/utils/mining"
	"github.com/frin-network/frind/util"
	"github.com/kaspanet/go-secp256k1"

	"github.com/frin-network/frind/stability-tests/common"
	"github.com/frin-network/frind/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("frind-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("frind datadir: %s", dataDir)

	frindCmd, err := common.StartCmd("KASPAD",
		"frind",
	...
	spawn("startNode-frindCmd.Wait", func() {
		err := frindCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("frindCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes frind closes ungracefully
				log.Errorf("frindCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(frindCmd, "frindCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	frinminerCmd, err := common.StartCmd("MINER",
		"frinminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(frinminerCmd, "frinminerCmd")
	return nil
}
