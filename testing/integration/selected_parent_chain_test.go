...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of frind instances
	frind1, frind2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := frind1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In frind2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, frind2)
	}

	// Connect the two frinds. This should trigger sync
	// between the two nodes
	connect(t, frind1, frind2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := frind1.rpcClient.GetVirtualSelectedParentChainFromBlock(
