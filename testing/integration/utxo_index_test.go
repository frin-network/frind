...
func TestUTXOIndex(t *testing.T) {
	...
	err := frind.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = frind.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := frind.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
