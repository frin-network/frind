...
// 2. A wrapper that implements isfrindMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `frindMessage_` prefix and `Request` suffix
...
// generateFrindMessage generates a wrapped FrindMessage with the given `commandValue`
func generateFrindMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.FrindMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	frindMessage := reflect.New(reflect.TypeOf(protowire.FrindMessage{}))
	frindMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return frindMessage.Interface().(*protowire.FrindMessage), nil
}
