...
import (
	...
	"github.com/frin-network/frind/infrastructure/os/execenv"
	"github.com/frin-network/frind/infrastructure/os/limits"
	"github.com/frin-network/frind/infrastructure/os/signal"
	"github.com/frin-network/frind/infrastructure/os/winservice"
...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "frindsvc",
	DisplayName: "Frind Service",
	Description: "Downloads and stays synchronized with the Kaspa blockDAG and " +
		"provides DAG services to applications.",
}

type frindApp struct {
	cfg *config.Config
}

// StartApp starts the frind app, and blocks until it finishes running
func StartApp() error {
...
func (app *frindApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start frind: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down frind...")
		...
		log.Infof("frind shutdown complete")
	}()
