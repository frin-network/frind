...
import (
	...
	"github.com/frin-network/frind/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		frindMessage, err := client.Post(&protowire.FrindMessage{Payload: &protowire.FrindMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := frindMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	frindMessage := &protowire.FrindMessage{}
	err := protojson.Unmarshal([]byte(response), frindMessage)
	if err != nil {
	...
	return marshalOptions.Format(frindMessage)
}
